 ...
 */
@Slf4j
public class JsonSession extends SimpleSession {
     ...
     * <li>If there is an internal system error and the session state can no longer accurately reflect the user's
     * behavior, such in the case of a system crash</li>
     * </ul>
     * <p/>
     * Once stopped, a session may no longer be used. It is locked from all further activity.
     *
     * @return Result The time the session was stopped, or <tt>null</tt> if the session is still active.
     */
    @Override
    ...
    /**
     * Returns true if this session has expired, false otherwise. If the session has expired, no further user
     * interaction with the system may be done under this session.
     *
     * @return Result true if this session has expired, false otherwise.
     */
    @Override
     ...
     *
     * @return Result true if the specified session has expired, false otherwise.
     */
    @Override
    ...
    public void validate() throws InvalidSessionException {
        ...
        if (isTimedOut()) {
            ...
            String msg =
                "Session with id [" + sessionId + "] has expired. " + "Last access time: " + df.format(lastAccessTime)
    ...
    private Map<Object, Object> getAttributesLazy() {
        ...
        if (attributes == null) {
            attributes = new HashMap<Object, Object>(12);
     ...
     * {@link #getId() id}s are equal. If the argument is a {@code SimpleSession} and either 'this' or the argument does
     * not yet have an ID assigned, the value of {@link #onEquals(SimpleSession) onEquals} is returned, which does a
     * necessary attribute-based comparison when IDs are not available.
     * <p/>
     * Do your best to ensure {@code SimpleSession} instances receive an ID very early in their lifecycle to avoid the
     * more expensive attributes-based comparison.
     *
     * @param obj the object to compare with this one for equality.
     * @return Result {@code true} if this object is equivalent to the specified argument, {@code false} otherwise.
     */
    @Override
    ...
    /**
     * Provides an attribute-based comparison (no ID comparison) - incurred <em>only</em> when 'this' or the session
     * object being compared for equality do not have a session id.
     *
     * @param ss the SimpleSession instance to compare for equality.
     * @return Result true if all the attributes, except the id, are equal to this object's attributes.
     * @since 1.0
     */
    @Override
    ...
    /**
     * Returns the hashCode. If the {@link #getId() id} is not {@code null}, its hashcode is returned immediately. If it
     * is {@code null}, an attributes-based hashCode will be calculated and returned.
     * <p/>
     * Do your best to ensure {@code SimpleSession} instances receive an ID very early in their lifecycle to avoid the
     * more expensive attributes-based calculation.
     *
     * @return Result this object's hashCode
     * @since 1.0
     */
    @Override
    public int hashCode() {
     ...
     *
     * @return Result the string representation of this SimpleSession, equal to
     *         <code>getClass().getName() + &quot;,id=&quot; + getId()</code>.
     * @since 1.0
     */
    @Override
