...
/**
 * @author zhaojianmou
 * @date 2025/12/10 15:25:29
 * @title
 * @description
 */
@Slf4j
@Component
public class SignAuthHandleUtils {
...
//            String decrypt = new String(Base64Utils.decodeFromString(params));
            ...
            if (sysApiSystem.getSignType().contains(LoginEncryptionConstant.SM2.toUpperCase())) {
                return Sm2Util.sm2CheckContent(appId + params, sign, sysApiSystem.getPublicKey());
            } else if (sysApiSystem.getSignType().contains(LoginEncryptionConstant.RSA.toUpperCase())) {
                return SignatureUtils.rsaCheckContent(appId + params, sign, sysApiSystem.getPublicKey());
            }
            return false;
        } catch (Exception e) {
            log.error("系统异常",e);
