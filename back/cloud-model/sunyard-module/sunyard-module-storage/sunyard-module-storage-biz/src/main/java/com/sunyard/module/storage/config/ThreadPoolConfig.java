...
public class ThreadPoolConfig {
    ...
    @Value("${fileCacheThreadPool.corePoolSize:8}")
    private int corePoolSize;
    /**
     * 最大线程数 2*CPU_NUM
     */
    @Value("${fileCacheThreadPool.maxPoolSize:16}")
    private int maxPoolSize;

    @Value("${fileCacheThreadPool.keepAliveSeconds:120}")
    private int keepAliveSeconds;

    @Value("${fileCacheThreadPool.queueCapacity:100}")
     ...
     */
    @Primary
    @Bean("FileCacheThreadPool")
    public TaskExecutor fileCacheThreadPool() {
        String threadName = "FileCache-thread-pool";
        log.info("开始初始化线程池:{}", threadName);
        ThreadPoolTaskExecutor taskExecutor = new ThreadPoolTaskExecutor();
