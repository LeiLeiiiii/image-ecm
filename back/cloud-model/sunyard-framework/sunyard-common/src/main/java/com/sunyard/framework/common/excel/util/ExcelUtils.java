package com.sunyard.framework.common.excel.util;
/*
 * Project: SunAM
 *
...
import com.sunyard.framework.common.excel.EasyExcelWriterFactory;
...
public class ExcelUtils {
     ...
     * @return Result List<Map<colNum, cellValue>>
     */
     ...
     * @param headRowNum 表头占的行数，从0开始（如果要连表头一起读出来则传0）
     */
    public static void asyncRead(InputStream inputStream, AnalysisEventListener<T> excelListener, Integer sheetNo,
        Integer headRowNum) {
     ...
     * @param headRowNum 表头占的行数，从0开始（如果要连表头一起读出来则传0）
     */
    public static void asyncRead(File file, AnalysisEventListener<T> excelListener, Integer sheetNo,
        Integer headRowNum) {
     ...
     * @param headRowNum 表头占的行数，从0开始（如果要连表头一起读出来则传0）
     */
    public static void asyncReadModel(InputStream inputStream, AnalysisEventListener<T> excelListener, Class clazz,
     ...
     * @param headRowNum 表头占的行数，从0开始（如果要连表头一起读出来则传0）
     */
    public static void asyncReadModel(File file, AnalysisEventListener<T> excelListener, Class clazz, Integer sheetNo,
        Integer headRowNum) {
     ...
     * 
     * @param filePath excel地址
     * @param templateFileName 临时文件名
     * @param headClazz 模型的类类型
     * @param data 数据
     */
    ...
    public static void writeExclude(String filePath, Class headClazz, List data, Set<String> excludeCols,
        Integer sheetNo, String sheetName) {
        EasyExcel.write(filePath, headClazz).includeColumnFieldNames(excludeCols).sheet(sheetNo, sheetName)
            .doWrite(data);
    }

    /**
     * 多个sheet页的数据链式写入 ExcelUtil.writeWithSheets(outputStream) .writeModel(ExcelModel.class, excelModelList,
     * "sheetName1") .write(headData, data,"sheetName2") .finish();
     * 
    ...
    /**
     * 多个sheet页的数据链式写入 ExcelUtil.writeWithSheets(file) .writeModel(ExcelModel.class, excelModelList, "sheetName1")
     * .write(headData, data,"sheetName2") .finish();
     * 
     * @param file excel文件
     * @return Result
     */
    ...
    /**
     * 多个sheet页的数据链式写入 ExcelUtil.writeWithSheets(filePath) .writeModel(ExcelModel.class, excelModelList, "sheetName1")
     * .write(headData, data,"sheetName2") .finish();
     * 
     * @param filePath excel地址
     * @return Result
    ...
    /**
     * 多个sheet页的数据链式写入（失败了会返回一个有部分数据的Excel） ExcelUtil.writeWithSheets(response, exportFileName)
     * .writeModel(ExcelModel.class, excelModelList, "sheetName1") .write(headData, data,"sheetName2") .finish();
     * 
     * @param response 响应头
