...
import com.sunyard.framework.onlyoffice.dto.documentfunction.FwAnonymous;
...
@Data
@Configuration
public class FwFileCustomization implements Serializable {
     ...
     */
    @Value("${onlyoffice.editor.customization.comments:true}")
    private Boolean comments;
    /**
     * 定义其他操作按钮是显示在编辑器窗口标题的上部徽标旁边 （false） 还是显示在工具栏 （true） 中，从而使标题更紧凑。 默认值为false。
     */
    @Value("${onlyoffice.editor.customization.compactHeader:false}")
    private Boolean compactHeader;
    /**
     * 定义显示的顶部工具栏类型是完整 （假） 还是紧凑 （真）。 默认值为false。
     */
    @Value("${onlyoffice.editor.customization.compactToolbar:false}")
    private Boolean compactToolbar;
    /**
     * 定义仅与 OOXML 格式兼容的功能的使用。 例如，不要对整个文档使用注释。 默认值为false。
     */
    @Value("${onlyoffice.editor.customization.compatibleFeatures:false}")
     ...
     * 示例：true;
     */
    @Value("${onlyoffice.customization.features:false}")
     ...
     */
    @Resource
    private FwFeedback fwFeedback;
    /**
     * 在文档编辑服务中保存文档时（例如，单击“保存”按钮等），
     * 将文件强制保存的请求添加到回调处理程序中。 默认值为false。
     */
    @Value("${onlyoffice.editor.customization.forcesave:false}")
     ...
     */
    @Resource
    private FwGoback fwGoback;

    /**
     * 定义是显示还是隐藏“帮助”菜单按钮。 默认值为true。
     */
    @Value("${onlyoffice.editor.customization.help:true}")
    private Boolean help;
    /**
     * 定义在首次加载时是显示还是隐藏注释面板。 默认值为false。此参数仅适用于演示文稿编辑器ppt。
     */
    @Value("${onlyoffice.editor.customization.hideNotes:false}")
    private Boolean hideNotes;
    /**
     * 定义首次加载时是显示还是隐藏右侧菜单。 默认值为false。
     */
    @Value("${onlyoffice.editor.customization.hideRightMenu:false}")
    private Boolean hideRightMenu;
    /**
     * 定义是显示还是隐藏编辑器标尺。 此参数可用于文档和演示文稿编辑器。文档编辑器的默认值为false，演示文稿的默认值为true。
     */
    @Value("${onlyoffice.editor.customization.hideRulers:false}")
     ...
     */
    @Resource
    private FwLogo fwLogo;

    /**
     * 定义在编辑器打开时是否自动运行文档宏。 默认值为true。false值对用户隐藏宏设置。
     */
    @Value("${onlyoffice.editor.customization.macros:false}")
     ...
     * warn 默认
     */
    @Value("${onlyoffice.editor.customization.macrosMode:warn}")
     ...
     */
    @Value("${onlyoffice.editor.customization.mentionShare:true}")
    private Boolean mentionShare;
    /**
     * 定义插件是否启动并可用。 默认值为true。
     */
    @Value("${onlyoffice.editor.customization.plugins:true}")
     ...
     */
    @Resource
    private FwReview fwReview;

    /**
     * 定义文档标题是在顶部工具栏上可见 （假） 还是隐藏 （真）。 默认值为false。
     */
    @Value("${onlyoffice.editor.customization.toolbarHideFileName:false}")
    private Boolean toolbarHideFileName;

    /**
     * 定义顶部工具栏选项卡是清晰显示 （false） 还是仅突出显示以查看选择了哪个选项卡 （true）。 默认值为false。
     */
    @Value("${onlyoffice.editor.customization.toolbarNoTabs:false}")
     ...
     */
    @Value("${onlyoffice.editor.customization.uiTheme:theme-dark}")
     ...
     * 默认值为厘米 （cm）。
     */
    @Value("${onlyoffice.editor.customization.uiTheme:cm}")
    private String unit;
     ...
     * 默认值为100。
     */
    @Value("${onlyoffice.editor.customization.zoom:100}")
