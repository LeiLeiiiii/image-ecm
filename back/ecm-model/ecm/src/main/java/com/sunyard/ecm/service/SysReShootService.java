 ...
 */
@Service
public class SysReShootService {
     ...
     * 查询翻拍资料类型树
     */
    public Result<List<EcmDocTreeDTO>> searchRemakeTypeTree(int state) {
        List<EcmDocDef> ecmDocDefs = ecmDocDefMapper.selectChildren(state,null,null,null,null);
    ...
    public Boolean updateRemakeState(List<EcmDocDef> vo) {
        if (!CollectionUtils.isEmpty(vo)){
            ...
            vo.forEach(ecmStateVo -> {
                ...
                wrapper.eq(EcmDocDef::getDocCode,ecmStateVo.getDocCode())
                        .set(EcmDocDef::getIsRemade, ecmStateVo.getIsRemade());
                ...
                docDef.setIsRemade(ecmStateVo.getIsRemade());
