...
CREATE TABLE `afm_api_data`  (
  ...
  `status` int NULL DEFAULT NULL COMMENT '队列处理状态，0未成功，1成功',
...
) ENGINE = InnoDB AUTO_INCREMENT = 1777966235326102314 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '对外接口请求数据' ROW_FORMAT = Dynamic;
...
CREATE TABLE `afm_file_exif`  (
  ...
  `server_id` bigint NULL DEFAULT NULL COMMENT '关联的服务器id',
...
CREATE TABLE `afm_image_ps_note`  (
  ...
  `upload_user_name` varchar(20) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '上传人（姓名）',
  `upload_org` varchar(20) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '上传机构',
  `ps_det_time` datetime NULL DEFAULT NULL COMMENT '篡改检测时间',
  `ps_det_result` int NULL DEFAULT NULL COMMENT '篡改检测结果（0无篡改 1有篡改）',
  `ps_count` int NULL DEFAULT NULL COMMENT '篡改处',
  `ps_det_file_id` bigint NULL DEFAULT NULL COMMENT '含篡改区域文件id，无篡改则为空',
  `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '更新时间',
  `update_time` datetime NULL DEFAULT NULL COMMENT '修改时间',
  `is_deleted` int NOT NULL DEFAULT 0 COMMENT '是否删除（0否 1是）',
  PRIMARY KEY (`id`) USING BTREE
...
CREATE TABLE `afm_invoice_det_note`  (
  ...
  `business_index` varchar(50) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '业务索引（主索引）',
  `material_type` varchar(50) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '资料类型',
  `exif_id` bigint NULL DEFAULT NULL COMMENT '文件id',
  ...
  `invoice_verify_result` int NULL DEFAULT NULL COMMENT '验真是否通过（0否 1是）',
  `invoice_dup_result` int NULL DEFAULT NULL COMMENT '查重是否通过 （0否 1是）',
  `invoice_link_result` int NULL DEFAULT NULL COMMENT '连续是否通过（0否 1是）',
  `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '更新时间',
  `update_time` datetime NULL DEFAULT NULL COMMENT '修改时间',
  `is_deleted` int NOT NULL DEFAULT 0 COMMENT '是否删除（0否 1是）',
  PRIMARY KEY (`id`) USING BTREE
...
CREATE TABLE `afm_invoice_det_note_assoc`  (
  `id` bigint NOT NULL COMMENT '主键',
  `invoice_note_id` bigint NULL DEFAULT NULL COMMENT '发票检测记录id',
  `assoc_exif_id` bigint NULL DEFAULT NULL COMMENT '关联文件id',
  `assoc_type` int NULL DEFAULT NULL COMMENT '关联文件类型（0重复 1连续）',
...
CREATE TABLE `afm_server`  (
  `id` bigint NOT NULL COMMENT '主键',
  `name` varchar(100) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '向量数据库服务名称',
  `host` varchar(100) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '连接地址',
  `port` varchar(20) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '端口',
  `user` varchar(32) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '连接用户名',
  `password` varchar(100) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '连接密码',
  `db_name` varchar(20) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '向量数据库的名称',
  `type` int NULL DEFAULT NULL COMMENT '状态 0:可读，1:可读写',
  `doc_code` varchar(32) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '资料类型',
  `status` int NULL DEFAULT NULL COMMENT '0:不可使用；1:可使用',
  `year` int NULL DEFAULT NULL COMMENT '年份，如果是多年则，存放第一年的日期',
  `num` int NULL DEFAULT NULL COMMENT '集合排列的数量',
  `collection_total` bigint NULL DEFAULT NULL COMMENT '集合内总条数',
  `collection_name` varchar(32) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '服务器对应集合名称',
  PRIMARY KEY (`id`) USING BTREE,
  UNIQUE INDEX `唯一索引`(`year` ASC, `doc_code` ASC, `status` ASC, `num` ASC, `type` ASC) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '服务器表' ROW_FORMAT = Dynamic;
