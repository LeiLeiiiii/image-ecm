...
//            String indexName = documentAnnotation.indexName();
    ...
    /***/
    public <T> List searchByPage(SearchSourceBuilder searchSourceBuilder, String indexName) {
        try {
            SearchRequest searchRequest = new SearchRequest();
            if (null != indexName) {
                searchRequest.indices(indexName);
            ...
            log.error("elasticsearch searchByPage错误:{}", e.getMessage(), e);
            throw new SunyardException(ResultCode.SYSTEM_ERROR, e.toString());
        }
    }
     ...
     * @return
     */
    public <T> Aggregations aggregate(SearchSourceBuilder searchSourceBuilder, String indexName) {
        try {
            SearchRequest searchRequest = new SearchRequest();
            if (null != indexName) {
                searchRequest.indices(indexName);
            ...
            log.error("elasticsearch aggregate错误:{}", e.getMessage(), e);
            throw new SunyardException(ResultCode.SYSTEM_ERROR, e.toString());
        }
    }
